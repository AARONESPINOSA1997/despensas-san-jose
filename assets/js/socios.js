...
async function cargarSocios(page = 1) {
  ...
  try {
    ...
    const response = await fetch(`http://localhost:3000/api/socios?${params}`, {
      headers: {
...
window.cambiarEstatus = async (id, estatusActual) => {
  ...
  try {
    const response = await fetch(`http://localhost:3000/api/socios/${id}/estatus`, {
      method: 'PATCH',
      headers: {
        'Content-Type': 'application/json',
        'Authorization': `Bearer ${sessionStorage.getItem('token')}`
...
window.eliminarSocio = async (id, numero, nombre) => {
  ...
  try {
    const response = await fetch(`http://localhost:3000/api/socios/${id}`, {
      method: 'DELETE',
      headers: {
        'Authorization': `Bearer ${sessionStorage.getItem('token')}`
      }
...
btnExportar.addEventListener('click', async () => {
  ...
  try {
    const response = await fetch('http://localhost:3000/api/socios?limit=999999', {
      headers: {
        'Authorization': `Bearer ${sessionStorage.getItem('token')}`
      }
    });
...
async function procesarExcel(event) {
  ...
  try {
    ...
    if (accion === 'reemplazar') {
      const response = await fetch('http://localhost:3000/api/socios/borrar-todos', {
        method: 'DELETE',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${sessionStorage.getItem('token')}`
